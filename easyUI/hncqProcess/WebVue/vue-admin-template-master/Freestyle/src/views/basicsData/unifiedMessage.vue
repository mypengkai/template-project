<template>
  <div class="p20">
    ...
    <el-pagination
      :current-page="formUnified.pageNo"
      :page-sizes="[10,20,30]"
      :page-size="formUnified.pageSize"
      :total="total"
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogTitle" width="30%" append-to-body>
      <el-form :model="formData" ref="formData" label-width="60px">
  ...
  methods: {
    ...
    deleteText(unifiedId) {
      this.$confirm("确定删除该条数据?", "提示", {
        ...
        unifiedMessage
          .deleteUnifiedApply({ unifiedId: unifiedId })
          .then(res => {
            if (res.data.ok) {
              this.initTable();
            }
