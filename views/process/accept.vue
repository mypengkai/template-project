  ...
  <div class="acceptLayout">
    ...
    <div class="section">
      ...
      <div v-show="isShowProjectItem" class="topBar">
        <el-form :inline="true" class="grid-content">
          ...
          <el-form-item>
            <div
              style="height:70vh;overflow-y:auto;border:1px solid #DCDFE6; width:19vw;border-radius: 5px"
            >
              <el-tree
                :data="projectItemTree"
                :props="defaultProjectItemProps"
                lazy
                :load="loadNextNode"
                highlight-current
    ...
    <div v-if="leftTopDetils" class="particularsList">
      ...
      <div v-if="leftTopDetils" class="Cztab">
        <el-table border :data="tableData" height="67vh" class="textList">
          ...
          <el-table-column fixed="right" label="操作" width="180" align="center">
            <template slot-scope="scope">
              <!-- 指定验收 -->
              <el-tooltip
                v-if="scope.row.adopt===null && scope.row.state1===0"
                class="item"
                effect="dark"
                content="指定验收计划"
                placement="top"
              ...
              <el-tooltip
                v-if="scope.row.adopt===null && scope.row.state1===1"
                class="item"
                effect="dark"
                content="修改验收计划"
                placement="top"
        ...
        <el-pagination
          ...
          @size-change="handleAppointProcessSizeChange"
          @current-change="loadAppointProcessList"
        />
      </div>
    </div>
    <!-- 添加工序弹框 -->
      ...
      <div class="blgx">
        ...
        >
          <div class="topBar">
            ...
            <div class="rl">
              <el-button
                ...
                class="pan-btn light-blue-btn"
                icon="el-icon-search"
                @click="selectCheckPerson(currentSelectedState)"
              >查询</el-button>
              <el-button
                type="primary"
                class="pan-btn light-blue-btn"
          ...
          <el-pagination
            ...
            @current-change="selectCheckPerson(currentSelectedState)"
          />
        ...
        <el-form
          ref="supplementProcessForm"
          :rules="supplementProcessRules"
          :model="formData"
          label-width="120px"
          size="mini"
        >
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="计划自检人:">
                <el-input
                  :readonly="true"
                  v-model="formData.planSelfCheckPersonName"
                  size="small"
                  style="width:100%"
                >
                  <el-button
                    slot="append"
                    icon="el-icon-search"
                    @click="selectCheckPerson('construction')"
                  ></el-button>
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="计划验收人:">
                <el-input
                  :readonly="true"
                  v-model="formData.planCheckPersonName"
                  size="small"
                  style="width:100%"
                >
                  <el-button
                    slot="append"
                    icon="el-icon-search"
                    @click="selectCheckPerson('supervisor')"
                  ></el-button>
                </el-input>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="计划自检时间:" prop="planSelfCheckTime">
                <el-date-picker
                  v-model="formData.planSelfCheckTime"
                  type="date"
                  placeholder="选择日期时间:"
            ...
            <el-col :span="12">
              <el-form-item label="计划验收时间:" prop="planCheckTime">
                <el-date-picker
                  v-model="formData.planCheckTime"
                  type="date"
                  placeholder="选择日期时间:"
                  value-format="yyyy-MM-dd"
                  format="yyyy-MM-dd"
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="实际自检时间:" prop="SelfCheckTime">
                <el-date-picker
                  v-model="formData.SelfCheckTime"
                  type="datetime"
                  placeholder="选择日期时间:"
          ...
          <el-row>
            <el-col :span="12">
              <el-button
                type="success"
                style="padding:2px;margin-left:50px;"
                @click="checkSelf"
              >快捷回复</el-button>
              <el-form-item label="自检描述:" prop="selfCheckDescribe">
                <el-input
                  :rows="2"
                  v-model="formData.selfCheckDescribe"
                  type="textarea"
                  placeholder="请输入内容"
                  style="width: 100%;"
                />
            ...
            <el-col :span="12">
              ...
              <el-form-item label="验收描述:" prop="checkDescribe">
                <el-input
                  :rows="2"
                  v-model="formData.checkDescribe"
                  type="textarea"
                  placeholder="请输入内容"
                  style="width: 100%;"
                />
            ...
            <el-col :span="12">
              <el-form-item label="验收图片:">
                <el-upload
                  ...
                  list-type="picture-card"
                  :auto-upload="false"
                  :before-upload="checkBeforeUplad"
        ...
        <div style="margin-left:60%;">
          ...
          <el-button
            type="primary"
            @click="subimitSupplementProcessFunction('supplementProcessForm')"
          >确 定</el-button>
        </div>
      </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="appointCheckDialogFormVisible" title="指定验收计划" width="40%">
      ...
      <el-form
        ref="apponitCheckFrom"
        :model="apponitCheckFrom"
        :rules="apponitCheckFromRules"
        label-width="120px"
      >
        <el-form-item label="自检人">
          <el-input
            :readonly="true"
            v-model="apponitCheckFrom.planSelfCheckPersonName"
            style="width:80%"
            size="small"
          >
            <el-button
        ...
        <el-form-item label="自检时间" prop="planSelfCheckTime">
          <el-date-picker
            :clearable="false"
            :editable="false"
            v-model="apponitCheckFrom.planSelfCheckTime"
            type="date"
            placeholder="选择日期"
            style="width:80%"
            value-format="yyyy-MM-dd"
        ...
        <el-form-item label="验收人">
          <el-input
            :readonly="true"
            v-model="apponitCheckFrom.planCheckPersonName"
            style="width:80%"
            size="small"
          >
        ...
        <el-form-item label="验收时间" prop="planCheckTime">
          <el-date-picker
            :clearable="false"
            :editable="false"
            v-model="apponitCheckFrom.planCheckTime"
            type="date"
            placeholder="选择日期"
            style="width:80%"
            value-format="yyyy-MM-dd"
...
export default {
  ...
  data() {
    ...
    return {
      startTime: {
        ...
        disabledDate: time => {
          if (this.apponitCheckFrom.planCheckTime) {
            return (
              time.getTime() >
              new Date(this.apponitCheckFrom.planCheckTime).getTime()
            );
          } else {
      ...
      endTime: {
        disabledDate: time => {
          if (this.apponitCheckFrom.planSelfCheckTime) {
            return (
              time.getTime() <
              new Date(this.apponitCheckFrom.planSelfCheckTime).getTime() -
                1 * 24 * 60 * 60 * 1000
            ); //可以选择同一天
          }
        }
      ...
      defaultProjectItemProps: {
        //工程分部分项
        children: "id",
        label: "projectItem",
        isLeaf: "leaf"
      },
      defaultProjectItemPropsBL: {
        //工程分部分项
        children: "id",
        label: "projectItem",
        isLeaf: "leaf"
      ...
      apponitCheckFromRules: {
        ...
        planCheckTime: [
          { required: true, trigger: "blur", message: "请选择监理验收时间" }
        ],
        planSelfCheckPerson: [
          { required: true, trigger: "blur", message: "请选择施工单位自检人" }
        ],
        planSelfCheckTime: [
          { required: true, trigger: "blur", message: "请选择施工单位自检时间" }
        ]
      },
      ...
      departname: "",
      projectItem: "",
      leakRepairProcessUrl:
        process.env.BASE_API + "/rest/processCheck/processAddImage", //补录工序url
      headers: {
  ...
  methods: {
    ...
    userGroupOnChange(data) {
      ...
      request
        .post("/rest/projectItemInfo/getProjectBQItemById", {
          userGroupId: data,
          pId: "0"
        })
        .then(res => {
    ...
    showSelectedProjectItemInfo(itemId) {
      // 点击树节点展示右边详情接口
      request
        .post("/rest/projectItemInfo/getProjectItemById/" + itemId)
        .then(res => {
          ...
          this.treeFrom.state1 =
            res.data.data.state1 === "1" ? "已指定验收" : "未指定验收";
        });
    },
    loadAppointProcessList() {
      //加载指定工序列表
      request
        .post("/rest/processCheck/getProject_ProcessPage", this.querydata)
        .then(res => {
          this.querydata.total = res.data.data.totalCount;
          this.tableData = res.data.data.data;
        });
    },
    handleAppointProcessSizeChange(val) {
      //分页时使用
    ...
    appointCheckPlan(data) {
      ...
      if (data.state1 == "1") {
        request
          ...
          .then(res => {
            this.apponitCheckFrom.planSelfCheckPersonName =
              res.data.data.selfCheckUser;
            this.apponitCheckFrom.planSelfCheckPerson =
              res.data.data.planSelfCheckPerson;
            this.apponitCheckFrom.planCheckPersonName = res.data.data.checkUser;
            this.apponitCheckFrom.planCheckPerson =
              res.data.data.planCheckPerson;
            this.apponitCheckFrom.planSelfCheckTime =
              res.data.data.planSelfCheckTime;
    ...
    submitCheckPlan(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          request
            .post(
              "/rest/processCheck/appointProcessCheckPersons",
              this.apponitCheckFrom
            )
            .then(res => {
    ...
    selfBeforeUplad(file) {
      //自检图片上传之前
      this.leakRepairUploadForm.append("files[]", file);
    },
    checkBeforeUplad(file) {
      //验收图片上传之前
      this.leakRepairUploadForm.append("files[]", file);
    },

    subimitSupplementProcessFunction(form) {
      //补录工序
      ...
      this.$refs[form].validate(valid => {
        if (valid) {
          request
            .post("/rest/processCheck/leakRepair", this.formData)
            .then(res => {
              if (res.data) {
