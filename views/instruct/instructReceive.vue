<template>
  <div class="p20">
    ...
    <!-- 查询列表 -->
    <el-table :data="sendCommandList" border class="textList" style="width: 100%" height="72vh">
      <el-table-column prop="projectitem" label="分部分项"/>
      <el-table-column prop="station" label="桩号" align="center" width="150"/>
      <el-table-column prop="launchPerson" label="发起人" width="100" align="center"/>
      <el-table-column prop="createTime" label="发起时间" width="150" align="center"/>
      <el-table-column prop="nowUser" label="处理人" width="100" align="center"/>
    ...
    />
    <!-- 编辑弹框 -->
    <el-dialog :visible.sync="dialogHandVisible" width="70%" class="dialogBox" title="指令查看">
      <orderInstruct :now-item="nowItem"/>
    </el-dialog>
    <!-- 指令处理 -->
    ...
    <!-- 指令退回 -->
    <el-dialog :visible.sync="dialogRenterVisible" width="50%" class="dialogBox" title="指令退回">
      <returnInstuct v-if="flag" :now-item="nowItem" @cancel="dialogRenterVisible=false" @comfirm="_searchList()"/>
    </el-dialog>
    <!-- 指令转发 -->
    <el-dialog :visible.sync="dialogTranspondVisible" width="50%" class="dialogBox" title="指令转发">
      <transpondInstruct v-if="flag" :now-item="nowItem" @cancel="dialogTranspondVisible=false" @comfirm="_searchList()"/>
    </el-dialog>
    <!-- 指令完成 -->
    <el-dialog :visible.sync="dialogDoneVisible" width="50%" class="dialogBox" title="指令完成">
      <doneInstrcut v-if="flag" :now-item="nowItem" @cancel="dialogDoneVisible=false" @comfirm="_searchList()"/>
    </el-dialog>
    <!-- 指令复核 -->
    <el-dialog :visible.sync="dialogOfficerVisible" width="50%" class="dialogBox" title="指令复核">
      <officeInstruct v-if="flag" :now-item="nowItem" @cancel="dialogOfficerVisible=false" @comfirm="_searchList()"/>
    </el-dialog>
  </div>
</template>
